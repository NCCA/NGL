...
class TestShaderLib(unittest.TestCase):
  ...
  def testLoadShader(self) :
    self.assertTrue(pyngl.ShaderLib.loadShader('Test',sourcedir+'vert.glsl',sourcedir+'frag.glsl',pyngl.ErrorExit.OFF))
  ...
  def testLoadErrorShader(self) :
    self.assertFalse(pyngl.ShaderLib.loadShader('Test',sourcedir+'vertErr.glsl',sourcedir+'fragErr.glsl',pyngl.ErrorExit.OFF))

  def testloadParts(self) :
    shaderName='Test2'
    pyngl.ShaderLib.createShaderProgram(shaderName,pyngl.ErrorExit.OFF)
    Vertex='Test2Vert'
    pyngl.ShaderLib.attachShader( Vertex, pyngl.ShaderType.VERTEX, pyngl.ErrorExit.OFF)
    pyngl.ShaderLib.loadShaderSource(Vertex,sourcedir+'vert.glsl')
    self.assertTrue(pyngl.ShaderLib.compileShader(Vertex))

    Fragment='Test2Frag'
    pyngl.ShaderLib.attachShader( Fragment, pyngl.ShaderType.FRAGMENT, pyngl.ErrorExit.OFF)
  ...
  def testSetUniform(self) :
    ...
    self.assertTrue(pyngl.ShaderLib.loadShader(shaderName,sourcedir+'testUniformVertex.glsl',sourcedir+'testUniformFragment.glsl',pyngl.ErrorExit.OFF))
    pyngl.ShaderLib.use(shaderName);
    ...
    resultF1,resultF2,resultF3,resultF4=pyngl.ShaderLib.getUniform4f('testVec4')
    self.assertAlmostEqual(resultF1,0.5,delta=3)
    self.assertAlmostEqual(resultF2,2.0,delta=3)
    self.assertAlmostEqual(resultF3,-22.2,delta=3)
