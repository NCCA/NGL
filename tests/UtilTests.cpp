...
// these test were generated by co-pilot her is it's explination
// This code defines two unit tests for the generateDistinctColours 
// function using the Google Test framework.

// The first test checks that the function returns the correct number of colours
TEST(Util, generateDistinctColoursReturnsCorrectNumberOfColours) 
{
    auto colours = ngl::generateDistinctColours(10);
    EXPECT_EQ(colours.size(), 10);
}

// while the second test checks that the function returns distinct colours. 
// The second test uses a nested loop to compare each pair of colours generated by the function, 
// and checks that they are not equal using the EXPECT_NE macro.


TEST(Util, generateDistinctColoursReturnsDistinctColours) 
{
    auto colours = ngl::generateDistinctColours(100);
